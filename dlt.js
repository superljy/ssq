...
let ssqFetch = async () => {
    let response = await fetch(`http://apis.juhe.cn/lottery/history?key=${process.env.KEY}&lottery_id=dlt&page_size=50&page=1`);
...
ssqFetch().then((data) => {
        ...
        let newReds = noBlues(lottery_res);

        let newLottery = reduceDimension(newReds);
        ...
        fs.writeFile('dlt_numbers.txt', `${Number(lotteryList[0].lottery_no) + 1}期推荐: 红球 : ${finalReds} , 蓝球 : ${finalBlues}`, (err) => {
            if (err) throw err;
            console.log('done!');
        })
    })
...
function balls(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
        newBalls.push(arr[i].slice(5));
    }
    return reduceDimension(newBalls);
}

//获取截取蓝球后的新数组
function noBlues(arr) {
    let newReds = [];
    for (let i = 0; i < arr.length; i++) {
        arr[i].splice(5);
        newReds.push(arr[i]);
